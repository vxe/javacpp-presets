...
ESSENTIA_VERSION=2.0.1

download https://github.com/MTG/essentia/archive/v$ESSENTIA_VERSION.tar.gz essentia-$ESSENTIA_VERSION.tar.gz

mkdir -p "$PLATFORM$EXTENSION"
cd "$PLATFORM$EXTENSION"
INSTALL_PATH=`pwd`
echo "Decompressing archives..."
tar --totals -xzf ../opencv-$ESSENTIA_VERSION.tar.gz
tar --totals -xzf ../opencv_contrib-$ESSENTIA_VERSION.tar.gz

cd opencv_contrib-$ESSENTIA_VERSION
patch -Np1 < ../../../opencv_contrib.patch

cd ../opencv-$ESSENTIA_VERSION
...
case $PLATFORM in
    ...
    #     sedinplace "s/.so.${ESSENTIA_VERSION%-*}/.so/g" ../lib/cmake/opencv4/OpenCVModules-release.cmake
    #     ;;
    # linux-x86_64)
    ...
    #       CC="gcc -m64" CXX="g++ -m64" $CMAKE -DCMAKE_INSTALL_PREFIX="$INSTALL_PATH" -DCMAKE_INSTALL_LIBDIR="lib" -DCMAKE_CXX_FLAGS="-mpower8-vector" -DCMAKE_C_FLAGS="-mpower8-vector" $BUILD_X -DENABLE_PRECOMPILED_HEADERS=OFF $WITH_X $GPU_FLAGS -DCUDA_HOST_COMPILER="$(which g++)" $BUILD_CONTRIB_X
    #     else
    #       echo "Not native ppc so assume cross compiling"
    #       PKG_CONFIG_PATH=/usr/lib/powerpc64le-linux-gnu/pkgconfig/ CC="powerpc64le-linux-gnu-gcc" CXX="powerpc64le-linux-gnu-g++" $CMAKE -DCMAKE_INSTALL_PREFIX="$INSTALL_PATH" -DCMAKE_INSTALL_LIBDIR="lib" -DCMAKE_CXX_FLAGS="-mpower8-vector" -DCMAKE_C_FLAGS="-mpower8-vector" $BUILD_X -DENABLE_PRECOMPILED_HEADERS=OFF $WITH_X $GPU_FLAGS $BUILD_CONTRIB_X
    #     fi
    #     # download files CMake failed to download
    #     if [[ -f download_with_curl.sh ]]; then
    #         bash download_with_curl.sh
    ...
    macosx-*)
        ...
        make -j $MAKEJ
        make install/strip
        cp ../share/java/essentia/libessentia_java.dylib ../lib
        sedinplace "s/.${ESSENTIA_VERSION%-*}.dylib/.dylib/g" ../lib/cmake/essentia/EssentiaModules-release.cmake
        ;;
    # windows-x86)
...
rm ../java/org/essentia/android/AsyncServiceHelper.java
rm ../java/org/essentia/android/CameraBridgeViewBase.java
rm ../java/org/essentia/android/JavaCameraView.java
rm ../java/org/essentia/android/EssentiaLoader.java
rm ../java/*essentia* || true # remove stray binaries
